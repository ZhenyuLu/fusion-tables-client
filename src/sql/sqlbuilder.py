...
class SQL:
    ...
    def update(self, table_id, cols, values=None, row_id=None):
        ...
        if type(cols) == type({}):
            ...
            return "UPDATE %s SET %s WHERE ROWID = '%s'" % (table_id,
            ...
            return "UPDATE %s SET %s WHERE ROWID = '%s'" % (table_id, updateStatement, row_id)
    ...
    def delete(self, table_id, row_id):
        ...
        Returns:
          the sql statement
        """
        return "DELETE FROM %s WHERE ROWID = '%s'" % (table_id, row_id)
